...
export default function StampChainletter({ jobArg }) {
    ...
    async function run() {
      try {
        ...
        const r = await fetch(token.webhookUrl, {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${token.jwt}`, 'group-id': meta.chainletterCollection.id },
        });
        ...
        setInfo({ jobId: job.jobId, collectionId: meta.chainletterCollection.id, filesStamped: data.files_stamped });
        setStatus('done');
      } catch (e) {
        setError(e.message);
        setStatus('error');
  ...
  if (status === 'working') return <Box marginY={1}><Text color="yellow">Stamping collection…</Text></Box>;
  if (status === 'error')   return <Box marginY={1}><Text color="red">✖ {error}</Text></Box>;

  return (
    <Box flexDirection="column" marginY={1}>
      <Text color="green" bold>✔ Blockchain stamp submitted</Text>
